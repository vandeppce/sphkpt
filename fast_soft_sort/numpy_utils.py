...
from .third_party import isotonic_utils
import numpy as np
...
def isotonic_l2(input_s, input_w=None):
  ...
  """
  if input_w is None:
    input_w = tf.keras.backend.arange(len(input_s))[::-1] + 1
  input_w = tf.cast(input_w, input_s.dtype)
  solution = tf.zeros_like(input_s)
  # isotonic_utils.isotonic_l2(input_s - input_w, solution)
  solution, _ = tf.nn.isotonic_regression(input_s - input_w)
  return solution
...
def isotonic_kl(input_s, input_w=None):
  ...
  isotonic_utils.isotonic_kl(input_s, input_w.astype(input_s.dtype), solution)
  return solution
...
def _partition(solution, eps=1e-9):
  ...
  for i in range(1, len(solution)):
    ...
    if tf.abs(tf.cast(solution[i], tf.float32) - tf.cast(solution[i - 1], tf.float32)) > eps:
      sizes.append(0)
    sizes[-1] += 1
  return sizes
...
class Isotonic(_Differentiable):
  ...
  def compute(self):
    ...
    if self.regularization == "l2":
      ...
      # self.solution_, _ = tf.nn.isotonic_regression(self.input_s - self.input_w)
    else:
  ...
  def vjp(self, vector):
    start = 0
    # return_value = np.zeros_like(self.solution_)
    # print(self.solution_)
    return_value = tf.ones_like(self.solution_)
    for size in _partition(self.solution_):
      end = start + size
      if self.regularization == "l2":
        val = 1. / size
      ...
      for i in range(start, end):
        slice_tensor = tf.gather(vector, [j for j in range(start, end)])
        return_value = tf.tensor_scatter_nd_update(return_value, [[i]], [val * tf.reduce_sum(slice_tensor)])
...
def _inv_permutation(permutation):
  ...
  inv_permutation = tf.zeros([permutation.shape[0]], dtype=tf.int32)
  '''
  a = []
  for i in range(6):
    a.append(0)
  x = tf.gather(permutation, 1)
  # a[x] = 1
  a = tf.tensor_scatter_nd_update(a, [[3]], [1])
  '''
  
  for i in range(512):
    inv_permutation = tf.tensor_scatter_nd_update(inv_permutation, [[tf.gather(permutation, i)]], [i])
  # inv_permutation[permutation] = tf.keras.backend.arange(permutation.shape[0])
...
class Projection(_Differentiable):
  ...
  def compute(self):
    self.permutation = tf.argsort(self.input_theta)[::-1]
    input_s = tf.gather(self.input_theta, self.permutation)
    ...
    return tf.gather(primal_sol, self.inv_permutation)
  ...
  def vjp(self, vector):
    ...
    tmp = self.isotonic_.vjp(tf.gather(vector, self.permutation))
    ret -= tf.gather(tmp, self.inv_permutation)
    print(ret)
    return ret
...
class SoftRank(_Differentiable):
  ...
  def __init__(self, values, direction="ASCENDING",
    ...
    self.values = values
    self.input_w = tf.keras.backend.arange(512)[::-1] + 1
...
class SoftSort(_Differentiable):
  ...
  def compute(self):
    ...
    input_w = tf.cast(tf.keras.backend.arange(1, size + 1)[::-1], tf.float32) / self.regularization_strength
    values = self.sign * self.values
    self.permutation_ = tf.argsort(values)[::-1]
    s = tf.gather(values, self.permutation_)
  ...
  def vjp(self, vector):
    ...
    return tf.gather(self.isotonic_.vjp(vector), inv_permutation)
