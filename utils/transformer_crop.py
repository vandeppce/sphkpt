...
def transformer_crop(images_and_keypoints, out_size, kpts_scale=None, kpts_ori=None, thetas=None, name='SpatialTransformCropper'):
    # images : [B,H,W,C]
    ...
    images = images_and_keypoints[0]
    kpts_xy = images_and_keypoints[1]
    ...
    with tf.name_scope(name):
        ...
        if thetas is None:
            ...
            thetas = tf.concat([thetas, ones], axis=1) # [num_kp, 3,3]                                               #[256, 3, 3]
        ...
        x = tf.slice(T_g, [0,0,0], [-1,1,-1]) # [num_kp,1,H*W]                                                            # [256, 1, 256]
        ...
        kp_y_ofst = tf.expand_dims(tf.slice(kpts_xy, [0,1], [-1,1]), axis=1) # [B*K,1,1]                    # [256, 1, 1]
        ...
        # batch_inds  = tf.cast(tf.reshape(tf.split(kpts_xy, num_or_size_splits = 2, axis = 1)[0], [-1]), tf.int32)
        # batch_inds  = tf.cast(tf.split(kpts_xy, num_or_size_splits = 2, axis = 1)[0], tf.int32)
        
        # base = tf.tile(batch_inds[:,None], [1, out_height*out_width]) # [B*K,out_height*out_width]
        # base = tf.reshape(base, [-1]) * dim1
        
        batch_inds = K.arange(0, num_kp) * height * width
        batch_inds = K.expand_dims(batch_inds, axis = -1)
        base = K.repeat_elements(batch_inds, out_height * out_width, axis = 1)
        base = K.flatten(base)
        ...
        images = tf.split(images, num_or_size_splits = 2, axis = 0)
        images = tf.concat([tf.tile(image, [tf.cast(num_kp / num_batch, tf.int32), 1, 1, 1]) for image in images], axis = 0)

        # images = tf.tile(images, [num_kp, 1, 1, 1])
        ...
        output = tf.reshape(output, tf.stack([num_batch, tf.cast(num_kp / num_batch, tf.int32), out_height, out_width, C]))
        # output.set_shape([3, 256, out_height, out_width, images.shape[-1]])
